<template>
  <div class="IndicationColor mb-2">
    <div class="p-2 text-dark">
      <div class="row">
        <div
          class="col-md-10 pointing"
          data-bs-toggle="collapse"
          :data-bs-target="'#date' + open.id"
          aria-expanded="true"
      ...
      <div class="row">
        ...
        >
          ...
          <select
            ...
            v-model="open.fieldmodalname"
            @change="fieldmodalselect"
          >
            <option value="all">
              Select
            </option>
            <option
              v-for="input in inputFields.fields"
              v-bind:value="input.model"
              v-if="input.id < open.id"
            >
              {{input.model }}
            </option>
          </select>
        ...
        >
          ...
          <input
            type="text"
            class="form-control"
            @keyup="fieldmodalselect"
            v-model="open.isvisiblevalue"
          />
        </div>
      </div>
      <br />
...
export default {
    ...
    DeleteItem () {
      swal({
        ...
        if (willDelete) {
          ...
          this.delete = true;
          this.getDateDataModel(this.delete, this.open);
        } else {
  ...
  mounted () {
    ...
    var that = this;
    var thismodel = that.open.model;
    this.open.set = function (model, val) {
      var today = new Date(val);
      var dd = String(today.getDate()).padStart(2, "0");
      var mm = String(today.getMonth() + 1).padStart(2, "0");
      var yyyy = today.getFullYear();
      today = mm + "/" + dd + "/" + yyyy;
      model[thismodel] = today;
